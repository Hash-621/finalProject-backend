...
@RequestMapping("/api/v1/tour") // 3. 기본 URL 경로 설정 (예: /api/v1/community)
public class TourController {
    ...
    public ResponseEntity<?> getHospitalList() {
        ...
        return ResponseEntity.ok("관광지 목록 조회 성공");
    }
    ...
    public ResponseEntity<?> getHospitalDetail(@PathVariable Long id) {
        ...
        return ResponseEntity.ok("관광지 상세 조회 성공: " + id);
    }

    // 3. 등록 (POST)
    @PostMapping
    public ResponseEntity<?> createHospital(@RequestBody Map<String, Object> params) {
        // tourService.save(params);
        return ResponseEntity.ok("관광지 등록 성공");
    }
